...
- **[Kolmogorov flow with neural ODEs](kolmogorov/)**: write a pseudospectral Navier-Stokes solver for a chaotic 2D turbulent flow, then learn its dynamics as a convolutional latent neural ODE, benchmarked per lead time against persistence and climatology the way a weather model is (forecast skill, anomaly correlation). Part 1 learns a single flow at fixed viscosity; Part 2 conditions on viscosity as an input and generalizes across a range of Reynolds numbers, with an ablation confirming the model genuinely uses the viscosity it's given rather than inferring it from the initial condition. This project sits where my numerics background meets scientific machine learning, the learned-dynamics problem underlying weather and climate emulation.
