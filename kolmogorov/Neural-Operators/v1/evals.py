...
from fno import (FNO, KolmogorovDataset, standardise_nu, unstandardise_nu, HERE, device)
...
#the nu question carries over from the neural ODE: the model is *conditioned* on nu, but does
#it use it? amplitude used to scale like 1/nu under a single global sigma, so x0 leaked nu and
#the input was redundant. windows are normalised individually, which removes the scale cue
#(though not the spectral one, since low nu still means finer structure). so we score every
#nu separately, then feed deliberately wrong nu to see whether anything changes

#one thing does NOT carry over: the neural ODE paid an autoencoder reconstruction cost at lag 0
#(0.0185 MSE, the floor under everything else). the FNO works in physical space and its rollout
#is seeded with x0 itself, so lag 0 is exactly 0 MSE and exactly 1.0 ACC. that is an
#architectural difference, not a result, and the lag 0 row is not comparable across the two
WINDOW = 30
#kept at 10 rather than the 20 the fno.py test loader uses, so these numbers sit on the same
#window grid as the v2 benchmarks they are being compared against
STRIDE = 10
#must match the dt fno.py trains with, not the true 15/149 spacing of the data, because the
...
#the experiment the neural ODE cannot run at all. a stride-2 conv stack is defined in grid cells,
#so v2 is welded to 64x64; the spectral layers here are defined in wavenumbers, which mean the
#same thing on any grid. train at 64, score against 128 truth, and ask whether the learned
#operator transferred or whether it only ever learned a 64x64 image-to-image map
DATASET_128 = os.path.join(HERE, "kolmogorov_128x128_dataset.pt")
GRID_128 = 128

def resample(field, H, W):
    #spectral (sinc) interpolation of a periodic band-limited field onto a different grid.
    #bilinear would smear the k=4 structure; zero-padding the spectrum is exact for anything
    #band-limited below the source Nyquist, which both the forcing and the climatology are.
    #the scale factor is bookkeeping: irfft2 divides by the NEW N^2, so without it the field
    #comes back attenuated by (old N / new N)^2
    scale = (H * W) / (field.shape[-2] * field.shape[-1])
    return torch.fft.irfft2(torch.fft.rfft2(field), s=(H, W)) * scale

def load_model_at(ckpt_path, grid):
    #the one part of this model that is NOT resolution independent is the learned forcing, which
    #is stored per pixel. it is band-limited in practice (the probe found its power at (kx=0,
    #ky=4) and (0,8)), so it moves onto the finer grid exactly by zero-padding its spectrum.
    #every other parameter - spectral weights, 1x1 convs, nu embedders - loads unchanged, which
    #is precisely the claim being tested
    state = torch.load(ckpt_path, map_location="cpu")
    for key in [k for k in state if k.endswith("forcing")]:
        if state[key].shape[-1] != grid:
            state[key] = resample(state[key], grid, grid)
    model = FNO(1, grid).to(device)
...
def load_model(ckpt_path):
    ...
    #eval() is load bearing here, not just habit: FNO.forward branches on self.training to decide
    #whether to gradient checkpoint the rollout, and we want the plain path under no_grad
    model.eval()
    return model

def evaluate(model, indices=None, nu_map=None, shuffle=False, data=None):
    ...
    #data overrides the (dataset, climatology) pair so the 128x128 transfer test scores through
    #exactly this code rather than a near-copy of it. every accumulator below is shaped off the
    #data it sees, so nothing here is tied to 64x64
    dataset, climatology = data if data is not None else eval_data()
...
def plot_curves(res, out_dir):
    ...
    for values, name, color in [(res["mse_model"], "FNO", SERIES[0]),
    ...
    ax_acc.set_title("Anomaly correlation of the FNO", color=INK, fontsize=11,
                     loc="left", pad=10)
...
def plot_fields(res, out_dir):
    ...
    rows = [("Truth", truth, DIVERGING, -scale, scale),
            ("FNO", pred, DIVERGING, -scale, scale),
...
def plot_spectra(res, out_dir):
    ...
    for i, lag in enumerate(lags):
        ...
        ax.loglog(k, res["spec_pred"][lag][1:cutoff + 1], color=SERIES[1], linewidth=2,
                  label="FNO", solid_capstyle="round")
...
def plot_ablation(res_by_nu, res_wrong_by_nu, out_dir):
    #the load bearing figure for the whole conditioning claim: how much worse does the rollout
    #get when the model is handed the wrong nu? near zero means the nu input is decorative and
    #the model is reading nu off x0's own spectral shape instead
    #
...
def plot_transfer(res64, res128, out_dir):
    fig, (ax_skill, ax_spec) = plt.subplots(1, 2, figsize=(11, 4.2), facecolor=SURFACE)

    t = res64["t"]
    for res, name, color in [(res64, "64x64 (trained)", SERIES[0]),
                             (res128, "128x128 (zero shot)", SERIES[1])]:
        ax_skill.plot(t, res["mse_model"] / res["mse_clim"], color=color, linewidth=2,
                      label=name, solid_capstyle="round")
    ax_skill.axhline(1.0, color=MUTED, linewidth=0.9, linestyle="--")
    ax_skill.annotate("climatology", (float(t[-1]), 1.0), color=MUTED, fontsize=8,
                      ha="right", va="bottom")
    ax_skill.set_xlabel("lead time t", color=INK_2, fontsize=9)
    ax_skill.set_ylabel("MSE / climatology MSE", color=INK_2, fontsize=9)
    ax_skill.set_title("Does skill survive a change of grid?", color=INK, fontsize=11, loc="left")
    ax_skill.legend(frameon=False, fontsize=8, labelcolor=INK_2)
    style_axes(ax_skill)

    #the 128 grid dealiases at k=42 where 64 dealiased at 21, so every shell past 21 is content
    #the model was never trained on. leaving it empty is honest smoothing; filling it is aliasing
    lag = len(t) - 1
    k = torch.arange(res128["spec_true"].shape[-1])
    ax_spec.loglog(k[1:], res128["spec_true"][lag][1:], color=INK_2, linewidth=1.6, label="truth")
    ax_spec.loglog(k[1:], res128["spec_pred"][lag][1:], color=SERIES[1], linewidth=1.6,
                   label="prediction")
    ax_spec.axvline(21, color=MUTED, linewidth=0.9, linestyle="--")
    ax_spec.annotate(" beyond the 64x64 band", (21, ax_spec.get_ylim()[0]), color=MUTED,
                     fontsize=8, ha="left", va="bottom")
    ax_spec.set_xlabel("wavenumber k", color=INK_2, fontsize=9)
    ax_spec.set_ylabel("enstrophy", color=INK_2, fontsize=9)
    ax_spec.set_title(f"128x128 spectrum at t = {float(t[lag]):.1f}", color=INK,
                      fontsize=11, loc="left")
    ax_spec.legend(frameon=False, fontsize=8, labelcolor=INK_2)
    style_axes(ax_spec)

    fig.tight_layout()
    path = os.path.join(out_dir, "resolution_transfer.png")
    fig.savefig(path, dpi=160, facecolor=SURFACE)
    plt.close(fig)
    return path

def report_transfer(ckpt_path):
    #scored against the model's own 64x64 numbers rather than against a figure quoted from some
    #other run, so "did it transfer" has a control on the same checkpoint and the same code path
    _, clim64 = eval_data()
    res64 = evaluate(load_model(ckpt_path))

    ds128 = KolmogorovDataset(torch.load(DATASET_128), WINDOW, STRIDE, DT, False)
    #the climatology is resampled from the 64 one rather than rebuilt at 128: it has to be the
    #same physical baseline for the skill columns to compare, and rebuilding it out of the test
    #set would mean scoring against a baseline fitted to the thing being scored
    data128 = (ds128, resample(clim64, GRID_128, GRID_128))
    model128 = load_model_at(ckpt_path, GRID_128)
    res128 = evaluate(model128, data=data128)

    out_dir = os.path.join(BENCHMARKS, "resolution_transfer")
    os.makedirs(out_dir, exist_ok=True)
    t = res64["t"]

    lines = [f"resolution transfer: {os.path.basename(ckpt_path)}, trained at 64x64",
             f"64x64: {res64['windows']} windows    128x128: {res128['windows']} windows", "",
             "                 64x64 (trained)         128x128 (zero shot)",
             "    t       MSE   skill     ACC       MSE   skill     ACC"]
    for i in list(range(0, len(t), 3)) + [len(t) - 1]:
        a, b = res64, res128
        lines.append(f" {float(t[i]):4.1f}   {a['mse_model'][i]:7.4f} "
                     f"{a['mse_model'][i]/a['mse_clim'][i]:7.3f} {a['acc'][i]:7.3f}   "
                     f"{b['mse_model'][i]:7.4f} {b['mse_model'][i]/b['mse_clim'][i]:7.3f} "
                     f"{b['acc'][i]:7.3f}")

    skill64 = res64["mse_model"].mean() / res64["mse_clim"].mean()
    skill128 = res128["mse_model"].mean() / res128["mse_clim"].mean()
    lines += ["", f"window mean skill: {skill64:.3f} at 64x64, {skill128:.3f} at 128x128 "
                  f"({skill128 / skill64 - 1:+.1%})"]
    if skill128 < 1.0:
        lines.append("  -> the operator transferred: it still beats climatology on a grid it "
                     "never saw")
    else:
        lines.append("  -> no transfer: on the finer grid the model is worse than predicting "
                     "the mean field")

    #extrapolated nu and extrapolated resolution could plausibly fail together, so split them
    lines += ["", "128x128 by nu:", "      nu   seen   windows      MSE    skill      ACC"]
    for nu, idx in nu_groups(ds128).items():
        r = evaluate(model128, indices=idx, data=data128)
        seen = "yes" if any(abs(nu - v) < 1e-6 for v in TRAIN_NUS) else "no"
        lines.append(f" {nu:7.4f}  {seen:>5}  {r['windows']:9d}  {r['mse_model'].mean():8.4f} "
                     f"{r['mse_model'].mean()/r['mse_clim'].mean():8.3f} {r['acc'].mean():8.3f}")

    #the spectrum answers the question the MSE cannot: the 128 data carries enstrophy out to
    #k=42, but the model only ever saw k<=21 and its spectral weights only span k<=11
    lag = len(t) - 1
    st, sp = res128["spec_true"][lag], res128["spec_pred"][lag]
    lines += ["", f"at t = {float(t[lag]):.1f}, share of enstrophy above k = {SMALL_SCALE_K}:  "
                  f"truth {small_scale_fraction(st):.4f}, prediction "
                  f"{small_scale_fraction(sp):.4f}",
              f"share above k = 21 (outside the 64x64 band):  truth {st[22:].sum()/st.sum():.4f}, "
              f"prediction {sp[22:].sum()/sp.sum():.4f}"]

    report_text = "\n".join(lines)
    print("\n" + report_text)
    with open(os.path.join(out_dir, "summary.txt"), "w") as fh:
        fh.write(report_text + "\n")
    for path in (plot_transfer(res64, res128, out_dir), plot_fields(res128, out_dir),
                 plot_spectra(res128, out_dir)):
        print(f"wrote {os.path.relpath(path, HERE)}")
    print(f"wrote {os.path.relpath(out_dir, HERE)}/summary.txt\n")
...
def report(ckpt_path):
    ...
    #v2's lag 0 was decoder(encoder(x0)), so it was nu independent but still carried the
    #autoencoder reconstruction error. the FNO seeds its rollout with x0 itself, so lag 0 is the
    #input verbatim: exactly 0 MSE, exactly 1.0 ACC, and no floor under the later lags. anything
    #other than 0 there means the rollout is not being seeded the way forward() claims
    lines.append("(lag 0 is the input verbatim: 0 MSE by construction, so nu cannot enter and "
                 "there is no autoencoder floor as there was in v2)")
    ...
    if penalty < 0.02:
        lines.append("  -> the nu input is doing essentially nothing; the model is reading nu "
                     "off x0 itself. the conditioning claim is NOT supported")
    elif penalty < 0.15:
...
if __name__ == "__main__":
    for checkpoint in get_checkpoints():
        report(checkpoint)
    #the resolution transfer test runs once, on the checkpoint worth quoting, and only if the
    #128x128 set has been generated. it is a property of the architecture, not of training, so
    #there is nothing to learn from sweeping it across epochs
    if os.path.exists(DATASET_128):
        report_transfer(os.path.join(HERE, "model_best.pth"))
    else:
        print(f"skipping resolution transfer: {os.path.basename(DATASET_128)} not found")
