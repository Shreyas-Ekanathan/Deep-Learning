...
#datagen time
#lets get 200 initial conditions, 300 samples per for trianing, 20 extra for testing
samples = []
for i in range(220):
    ...
    omega0 = odeint(RHS, omega0, t)[-1] #integrate until stabilized, and then proceed
    
    #now get samples
    T = 15
    num_snapshots = 300
    save_pts = torch.linspace(0, T, num_snapshots)
    omega = odeint(RHS, omega0, save_pts)
    
    samples.append(omega)
    
all_runs = torch.stack(samples[0:200])
torch.save(all_runs, os.path.join(HERE, "kolmogorov_train_dataset.pt"))

all_runs = torch.stack(samples[200:220])
torch.save(all_runs, os.path.join(HERE, "kolmogorov_test_dataset.pt"))
