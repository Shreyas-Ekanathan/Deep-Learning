...
#add force to the data for training
class molecule():
...
class DATASET(Dataset):
    def __init__(self, n=25000, energy_mean=None, energy_std=None, force_std=None):
        self.examples = []
        for i in range(n):
            species, positions = generate_random_configs()
            target_energy = compute_energy(positions, species)
            target_force = compute_forces(positions, species)
            self.examples.append((species, positions, target_energy, target_force))
        ...
        for i in range(n // 100): #100 * 100 = 10000 more samples
            ...
            for j in range(len(pos_vector)):
                ...
                target_force = compute_forces(pos_vector[j], species)
                self.examples.append((species, pos_vector[j], target_energy, target_force))

        self.holdout_trajectory = gen_trajectory()

        energies = np.array([e for _, _, e, _ in self.examples])
        forces = np.array([f for _, _, _, f in self.examples])
        ...
        # forces are zero-mean (newtons 3rd law)
        self.force_std = force_std if force_std is not None else forces.std()

    def __len__(self):
        return len(self.examples)

    def __getitem__(self, idx):
        species, positions, target_energy, target_force = self.examples[idx]
        ...
        normalized_force = target_force / self.force_std
        return node_features, np.float32(positions), np.float32(normalized_energy), np.float32(normalized_force)
